      ...
      if (listResult.length) {
        ...
        if (extraDiscountValue) {
          if (this.localCouponCode) {
            ...
            if (this.localCouponCode.toLowerCase() === 'livelo') {
              this.alertText = 'Cupom Ativado! Você ganha pontos LIVELO nessa compra.'
            } else {
              this.alertText = this.i19couponAppliedMsg + '*O valor do desconto não é cumulativo com outras promoções'
            }
          } else {
            this.alertText = this.i19campaignAppliedMsg + '*O valor do desconto não é cumulativo com outras promoções'
          }
